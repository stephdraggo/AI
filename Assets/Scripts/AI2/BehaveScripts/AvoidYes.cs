...
namespace AI2
{
    [CreateAssetMenu(menuName = "Flock/Behaviour/AvoidanceYes")]
    [AddComponentMenu("AI/Flock Behaviour/AvoidanceYes")]
    public class AvoidYes : FilteredFlockBehaviour
    {
        public override Vector2 CalculateMove(FlockAgent agent, List<Transform> context, Flock flock)
        {
            ...
            {
                if (Vector2.SqrMagnitude(item.position - agent.transform.position) < flock.SquareAvoidanceRadius *10) //if the distance is less than the avoidance radius
                {
